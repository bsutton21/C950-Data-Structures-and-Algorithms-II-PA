    ...
    if len(truck_dist_list) == 0: # Base case returns the list if the list is empty
        ...
        try:
            ...
            for index in truck_dist_list:
                if current_distance(curr_loc, int(index[0])) <= low_val:
                    low_val = current_distance(curr_loc, int(index[0]))
                    new_loc = int(index[0])
                for index in truck_dist_list:
                    if current_distance(curr_loc, int(index[0])) == low_val:
                        if truck_num == 1:
                            first_opt_truck.append(index)
                            first_opt_truck_ind.append(index[0])
                            ...
                            second_opt_truck_ind.append(index[0])
                            pop_value = truck_dist_list.index(index)
                            truck_dist_list.pop(pop_value)
                            current_loc = new_loc
                            ...
                            third_opt_truck_ind.append(index[0])
                            pop_value = truck_dist_list.index(index)
                            truck_dist_list.pop(pop_value)
                            current_loc = new_loc
